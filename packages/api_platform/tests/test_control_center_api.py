"""RC1 Milestone 11 — Control Center thin API tests (P0-05 admin JWT)."""
...
@pytest.fixture()
def admin_ctx(client: TestClient) -> dict[str, str]:
    # Unique username per test function — shared persistence survives fixtures.
    from uuid import uuid4

    suffix = uuid4().hex[:8]
    user_id = f"u-cc-{suffix}"
    headers = admin_headers(
        client,
        user_id=user_id,
        username=f"ccadmin{suffix}",
    )
    return {"headers": headers, "user_id": user_id}


def test_schema_and_registry(client: TestClient, admin_ctx: dict[str, str]) -> None:
    headers = admin_ctx["headers"]
    client.cookies.clear()
    assert client.get("/api/v1/admin/control-center/schema").status_code == 401

    schema = client.get(
        "/api/v1/admin/control-center/schema",
        headers=headers,
    )
    ...
    registry = client.get(
        "/api/v1/admin/configuration/registry",
        headers=headers,
    )
    ...
    legacy = client.get("/api/v1/admin/configuration", headers=headers)
    assert legacy.status_code == 200
    assert legacy.json()["ok"] is True


def test_configuration_update_rollback(
    client: TestClient, admin_ctx: dict[str, str]
) -> None:
    headers = admin_ctx["headers"]
    updated = client.post(
    ...
    assert updated.json()["result"]["change"]["author"] == admin_ctx["user_id"]

    history = client.get(
        "/api/v1/admin/configuration/history",
        headers=headers,
    )
...
def test_feature_flags_business_rules_security(
    client: TestClient, admin_ctx: dict[str, str]
) -> None:
    headers = admin_ctx["headers"]
    flags = client.post(
    ...
    listed = client.get("/api/v1/admin/business-rules", headers=headers)
...
def test_module_posts_and_monitoring(
    client: TestClient, admin_ctx: dict[str, str]
) -> None:
    headers = admin_ctx["headers"]
    for path, payload in (
    ...
    mon = client.get("/api/v1/admin/monitoring", headers=headers)
    assert mon.status_code == 200
    assert mon.json()["ok"] is True

    audit = client.get("/api/v1/admin/audit/config", headers=headers)
    assert audit.status_code == 200
    assert isinstance(audit.json()["result"]["audit"], list)

    dash = client.get(
        "/api/v1/admin/control-center/dashboard",
        headers=headers,
    )
