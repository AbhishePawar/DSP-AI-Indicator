...
import type { CopilotResearchContext, ResearchCitationId } from "@/lib/copilot/types";
...
export function ResearchContextPanel({
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader title="Answer" description="Latest deterministic response" />
        <CardBody>
          {latestAnswer ? (
            <>
      ...
      <Card>
        ...
        <CardBody className="flex flex-wrap gap-2">
          ...
          <Link
            href={
              ticker ? `/analysis?symbol=${encodeURIComponent(ticker)}` : "/analysis"
            }
          >
            <Button size="sm" variant="secondary">
              Run New Analysis
          ...
          {context.canCompare ? (
            <Button size="sm" variant="secondary" type="button" onClick={onCompare}>
              Compare Companies
            </Button>
          ) : null}
        </CardBody>
