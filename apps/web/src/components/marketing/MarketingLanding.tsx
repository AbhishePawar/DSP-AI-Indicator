          ...
          <section className="flex flex-1 flex-col px-5 py-12 sm:px-8 lg:px-16 lg:py-16">
            <div className="mx-auto flex w-full max-w-[860px] flex-1 flex-col justify-center">
              ...
              <form onSubmit={submitResearch} className="relative rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 shadow-[var(--shadow-sm)] sm:p-5">
                ...
                {query.trim() ? <div className="relative z-10 mt-3 max-h-20 overflow-y-auto rounded-xl border border-[var(--border)] bg-[var(--surface)] shadow-[var(--shadow-md)]" role="listbox" aria-label="Company suggestions">
                  {matches.length ? matches.map((company) => <button key={company.ticker} type="button" role="option" aria-selected={selected?.ticker === company.ticker} onClick={() => { setSelected(company); setQuery(company.ticker); }} className="flex w-full items-center justify-between gap-3 border-b border-[var(--border)] px-3 py-2 text-left text-sm last:border-b-0 hover:bg-[var(--surface-2)]"><span><span className="block text-sm font-medium">{company.name}</span><span className="mt-1 block text-xs text-[var(--muted)]">{company.market}</span></span><ChevronRight className="size-4 text-[var(--muted)]" /></button>) : <div className="px-4 py-4 text-sm text-[var(--muted)]">No matching listed company found. Try a name, ticker or ISIN.</div>}
                </div> : null}
              </form>
