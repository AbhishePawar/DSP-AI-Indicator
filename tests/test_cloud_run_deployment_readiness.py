...
class TestStartApiPortBinding:
    def test_honors_cloud_run_port(self) -> None:
        ...
        assert 'PORT="${PORT:-${DSP_API_PORT:-8000}}"' in text assert '--port"${PORT}"' in text
    ...
    def test_binds_all_interfaces(self) -> None:
        text = _START_API.read_text(encoding="utf-8")
        assert 'HOST="${DSP_API_HOST:-0.0.0.0}"' in text assert"uvicorn api_platform.api.app:app" in text
...
class TestCloudBuildDeployWiring:
    def test_sets_production_upstox_database_and_region(self) -> None:
        ...
        assert "DSP_ENVIRONMENT=production"in text assert"DSP_REGION=ap-south-1"in text assert"DSP_INVESTMENT_DATA_PROVIDER=upstox"in text assert"DSP_DATABASE_URL=dsp-database-url:latest"in text assert"DSP_UPSTOX_ANALYTICS_TOKEN=dsp-upstox-analytics-token:latest" in text
        assert (
            "--add-cloudsql-instances=" "project-34de429e-3c43-4ae7-b75:asia-south1:dsp-postgres"
        ) in text
        assert "--port=8000" in text assert"--region=$_REGION"in text assert"_REGION: asia-south1" in text
    ...
    def test_injects_gemini_api_key_from_secret_manager_version_1(self) -> None:
        ...
        assert "GEMINI_API_KEY=dsp-gemini-api-key:1" in secret_arg assert"dsp-gemini-api-key:latest" not in secret_arg
        deploy_runtime = "\n".join(secrets + env)
        assert "DEFAULT_AI_PROVIDER"not in deploy_runtime assert"DSP_AI_DEFAULT_PROVIDER"not in deploy_runtime assert"AI_ENABLED"not in deploy_runtime assert"activation_ready" not in deploy_runtime
