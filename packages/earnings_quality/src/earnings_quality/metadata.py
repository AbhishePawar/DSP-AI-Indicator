"""Version and provenance metadata for Earnings Quality Intelligence."""
...
EARNINGS_QUALITY_VERSION = "0.1.0-earnings-quality"
FRAMEWORK_VERSION = "0.1.0-core"


@dataclass(frozen=True, slots=True)
class EarningsQualityMetadata:
    """Immutable provenance for an Earnings Quality analysis artifact."""
    ...
    def __post_init__(self) -> None:
        ...
        if not engine_version:
            raise EarningsQualityValidationError("metadata.engine_version is required")
        if not framework_version:
            raise EarningsQualityValidationError(
                "metadata.framework_version is required"
            )
        if not schema_version:
            raise EarningsQualityValidationError("metadata.schema_version is required")
