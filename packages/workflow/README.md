...
DSP Workflow Intelligence — models, assembler, engine, reporter (H1.0–H1.3)

## 2. Responsibilities

- Provide the `workflow` domain façade for DSP AI Indicator.
...
- `ReportingResult`
- `ReportingStatus`
- … and 25 more (see `__all__` in package `__init__.py`)
...
```python
from workflow import EngineContext
...
```bash
pytest packages/workflow/tests -q --import-mode=importlib -p no:cov
```
