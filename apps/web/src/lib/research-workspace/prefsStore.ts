...
export const useResearchWorkspacePrefsStore = create<ResearchWorkspacePrefsState>()(
  persist(
    (set, get) => ({
      ...
      isFavourite: (ticker) =>
        get().favourites.some((f) => f.ticker === ticker.trim().toUpperCase()),
      togglePinned: (ticker) =>
        set((s) => {
          const sym = ticker.trim().toUpperCase();
          if (!sym) return s;
      ...
      isPinned: (ticker) => get().pinnedTickers.includes(ticker.trim().toUpperCase()),
      addNote: (ticker, text) =>
        set((s) => {
      ...
      addTag: (ticker, label) =>
        set((s) => {
          ...
          if (
            s.tags.some(
              (t) =>
                t.ticker === sym && t.label.toLowerCase() === trimmed.toLowerCase(),
            )
          ) {
            return s;
          }
