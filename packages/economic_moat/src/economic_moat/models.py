"""Immutable domain models for Economic Moat Intelligence (FEATURE-001)."""
...
from economic_moat.scoring import MoatDimension, MoatRating, MoatWeights
...
class EconomicConfidence:
    """Normalized confidence band for a moat conclusion (0.0–1.0)."""
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.value, (int, float)) or isinstance(self.value, bool):
            raise EconomicMoatValidationError("confidence.value must be numeric")
        ...
        if not basis:
            raise EconomicMoatValidationError("confidence.basis is required")
...
@dataclass(frozen=True, slots=True)
class EconomicScore:
    """Numeric moat score on a declared scale (default 0–100)."""
    ...
    def __post_init__(self) -> None:
        ...
        if not status:
            raise EconomicMoatValidationError("score.status is required")
        object.__setattr__(self, "status", status)
        if self.value is None:
            return
        if not isinstance(self.value, (int, float)) or isinstance(self.value, bool):
            raise EconomicMoatValidationError("score.value must be numeric or None")
        value = float(self.value)
        if not math.isfinite(value):
            raise EconomicMoatValidationError("score.value must be finite")
...
class EconomicEvidence:
    """Explainable evidence item backing a moat score."""
    ...
    def __post_init__(self) -> None:
        ...
        if not source:
            raise EconomicMoatValidationError("evidence.source is required")
        if not reference:
            raise EconomicMoatValidationError("evidence.reference is required")
        if not isinstance(self.confidence, (int, float)) or isinstance(
            self.confidence, bool
        ):
            raise EconomicMoatValidationError("evidence.confidence must be numeric")
...
@dataclass(frozen=True, slots=True)
class EconomicValidationSummary:
    """Immutable validation outcome for framework inputs and metadata."""
...
@dataclass(frozen=True, slots=True)
class EconomicExplainability:
    """Package-level explainability envelope for a moat analysis."""

    evidence: tuple[EconomicEvidence, ...] = ()
    confidence: EconomicConfidence = EconomicConfidence()
...
@dataclass(frozen=True, slots=True)
class MoatComponentScore:
    """One moat dimension with score, evidence, and Buffett-aligned reasoning."""
...
class EconomicAnalysis:
    """Complete Economic Moat analysis — explainable, evidence-backed."""

    metadata: EconomicMetadata
    validation: EconomicValidationSummary
    score: EconomicScore
    evidence: tuple[EconomicEvidence, ...]
    confidence: EconomicConfidence
    explainability: EconomicExplainability
    components: tuple[MoatComponentScore, ...] = ()
    overall_moat_rating: MoatRating | None = None
    summary: str = ""
    positive_factors: tuple[str, ...] = ()
    negative_factors: tuple[str, ...] = ()
    risks: tuple[str, ...] = ()
    recommendation: str = ""
    weights_used: MoatWeights | None = None
    ...
    def overall_moat_score(self) -> float | None:
        """Convenience alias for ``score.value``."""
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "score": self.score.to_dict(),
            "overall_moat_score": self.overall_moat_score,
            "overall_moat_rating": (
                self.overall_moat_rating.value
                if self.overall_moat_rating is not None
                else None
            ),
            "evidence": [item.to_dict() for item in self.evidence],
            "confidence": self.confidence.to_dict(),
