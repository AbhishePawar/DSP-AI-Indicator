...
def test_dsp_authority_preserved_after_test_assembly() -> None:
    base = _compose_package()
    package = _package_with_evidence()
    dsp = build_public_research_report(base)
    dsp_attached = build_public_research_report(package)
    assert dsp.valuation.intrinsic_value_per_share.value == (
        dsp_attached.valuation.intrinsic_value_per_share.value
    )
    assert dsp.recommendation.action == dsp_attached.recommendation.action
    prompt = build_private_research_prompt(package)
    draft = DeterministicCanonicalResearchAiPort().interpret(prompt)
...
def test_evidence_does_not_become_dsp_data() -> None:
    ...
    dsp_after = build_public_research_report(package)
    assert dsp_after.valuation.intrinsic_value_per_share.value == (
        dsp_before.valuation.intrinsic_value_per_share.value
    )
    prompt = build_private_research_prompt(package)
    draft = DeterministicCanonicalResearchAiPort().interpret(prompt)
    assembly = assemble_canonical_research(package, draft)
    report = assembly.report or dsp_after
...
def test_privacy_draft_and_public_report() -> None:
    ...
    dsp = str(build_public_research_report(package).to_public_dict()).lower()
    report = ""
    if assembly.report is not None:
