...
### 8.7 Workflow Automation (RC1 Milestone 5)

This addendum adds **Alert Rules** (Price / Valuation / Research Refresh / Earnings), **Scheduled Report** definitions, and a **Notification Center** as a new, additive package, `workflow_automation`, following the identical architecture Section 8.5 already established for `portfolio_store`: a `WorkflowAutomationStorePort` Protocol with an `InMemoryWorkflowAutomationStore` default and a `DatabaseWorkflowAutomationStore` over a duck-typed `production_platform.DatabasePort`. This is a **different** capability from the frozen `packages/workflow` (H1.0–H1.3 research-pipeline orchestration, which explicitly never schedules or persists) and from EPIC-A007's institutional approval workflow (`/workflow/action` draft→review→published) — both remain untouched. Every alert-rule evaluation reuses an already-frozen engine and never recomputes a signal itself: Price Alerts call `dsp_platform.market_quotes.get_authenticated_market_quote` (Data Connector Framework); Valuation Alerts call `dsp_platform.portfolio_intelligence_engine` (Section 8.6) with holdings sourced from `dsp_platform.portfolio_store_facade` (Section 8.5); Research Refresh Alerts do a simple date-difference comparison against a caller-supplied "last analysed at" timestamp; Earnings Alerts always evaluate to `Data unavailable.` because no earnings-calendar Data Connector Framework provider exists anywhere in the platform — the rule type exists in the schema only for forward extensibility, never fabricating a date. Because `dsp_platform` cannot import `production_platform`'s `SchedulerPort`/`JobQueuePort` (a forbidden import enforced by `dsp_platform.tests.test_boundaries`), and those ports are themselves documented as in-memory/process-local rather than restart-safe, this milestone does not introduce an autonomous scheduler — alert evaluation and scheduled-report execution are both caller-driven (`POST /workflow-automation/alerts/evaluate`, `POST /workflow-automation/schedules/{id}/run`), a recorded architectural gap rather than a silently-worked-around one. Scheduled Report content reuses the Portfolio Intelligence Engine and is serialized to JSON/CSV (pure formatting, not a new export engine). Notification delivery reuses the existing `auth.email_delivery.EmailProviderPort` (the same port used for password-reset/invite email) — no new delivery channel. Ten thin, additive, mostly-authenticated API routers are mounted at `/workflow-automation/*` (not `/workflow/*`, which the two existing capabilities above already occupy). On the frontend, a new `/workflow` route (`WorkflowAutomationWorkspace`) with three tabs — Alert Rules, Scheduled Reports, Notifications — follows the identical `ProtectedRoute` + `PageHeader` convention as `/portfolio`. See `docs/WORKFLOW_AUTOMATION_GUIDE.md` for the full architecture diagram, reuse table, and compliance table.
