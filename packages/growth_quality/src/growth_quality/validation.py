"""Input validation for Growth Quality Intelligence."""
...
from growth_quality.models import GrowthQualityValidationSummary

__all__ = ["validate_framework_inputs", "validate_metadata"]


def validate_metadata(metadata: object | None) -> GrowthQualityValidationSummary:
    if not isinstance(metadata, GrowthQualityMetadata):
        return GrowthQualityValidationSummary(
            ok=False,
            invalid_inputs=("metadata",),
            errors=("Invalid metadata: GrowthQualityMetadata is required",),
        )
    ...
    return GrowthQualityValidationSummary(ok=True, checks=("metadata_valid=True",))
