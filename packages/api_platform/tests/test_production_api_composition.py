...
→ ``run_execution_pipeline`` → authenticated valuation bundle →
configured investment adapters — and never by the legacy Yahoo/FRED
...
_SECRET = "investment-analytics-secret-token-not-real"
...
class TestProductionFailClosed:
    ...
    def test_missing_provider_reports_investment_fail_without_blocking_ready(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        platform = build_default_platform()
        monkeypatch.setenv("DSP_ENVIRONMENT", "production")
    ...
    def test_none_provider_selection_is_explicit_null(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setenv("DSP_ENVIRONMENT", "production")
    ...
    def test_no_in_memory_analysis_fallback_in_production(
        ...
        result = platform.health_check()
        inv = next(
            c for c in result.payload.checks if c.name == "investment_data_provider"
        )
        assert inv.status.value == "fail"
        assert "InMemory" not in inv.message
