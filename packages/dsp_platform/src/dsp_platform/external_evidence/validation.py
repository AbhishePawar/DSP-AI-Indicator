...
def assert_identities_compatible(
    ...
    subject_mic = normalize_identity_token(subject.mic) or None
    record_mic = normalize_identity_token(record_identity.mic) or None
    if subject_mic and record_mic and subject_mic != record_mic:
        raise ExternalEvidenceValidationError(
            "identity mismatch: MIC disagreement "
            f"({subject_mic!r} vs {record_mic!r}); "
...
def _validate_identity(identity: ExternalEvidenceIdentity) -> None:
    ...
    mic = identity.mic
    if mic is not None:
        if not isinstance(mic, str) or not mic.strip():
            raise ExternalEvidenceValidationError("identity rejected: empty MIC")
        compact_mic = mic.strip().upper()
        if compact_mic != mic:
            raise ExternalEvidenceValidationError(
                "identity rejected: MIC must be canonical uppercase "
                "without whitespace"
            )
        if len(compact_mic) != 4 or not compact_mic.isalnum():
            raise ExternalEvidenceValidationError(
                "identity rejected: MIC is present but not a "
