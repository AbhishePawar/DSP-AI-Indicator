...
class InMemoryTranscriptAdapter(TranscriptProviderPort):
    ...
    def get_transcripts(
        ...
        if not self.api_key:
            raise ProviderRequestError(
                "memory transcript adapter requires api_key (authentication)"
            )
        with self._lock:
    ...
    def health(self) -> ProviderHealth:
        return ProviderHealth(
            ...
            detail=(
                "seeded in-memory authenticated transcripts"
                if self.api_key
...
class FinancialModelingPrepTranscriptAdapter(TranscriptProviderPort):
    ...
    def get_transcripts(
        ...
        if not self.api_key.strip():
            raise ProviderRequestError(
                "financial modeling prep transcript adapter requires api_key"
            )
        ...
        if query.year is not None and query.quarter is not None:
            ...
            index_payload = self._client().get_json(
                self.dates_url, params={"symbol": symbol, "apikey": self.api_key}
            )
        ...
        return build_transcripts_bundle_from_mapping(
            symbol=symbol, transcripts=transcripts, provenance=provenance
        )
...
def build_default_transcript_registry_from_env() -> (
    PriorityProviderRegistry[TranscriptProviderPort]
):
