...
`POST /api/v1/auth/rbac/refresh` rotates the refresh token on every call and revokes the whole session if a stale (already-rotated) token is ever replayed — no configuration required, see [ENTERPRISE_AUTH_PLATFORM.md §3e](security/ENTERPRISE_AUTH_PLATFORM.md) and [SECURITY_GUIDE.md](SECURITY_GUIDE.md).

**Enterprise multi-provider auth** (Google / Microsoft / Facebook / India OTP / email lifecycle / admin IAM / TOTP MFA / Passkey) extends A009 without Auth.js — see [AUTH_ENTERPRISE_PLATFORM.md](AUTH_ENTERPRISE_PLATFORM.md) and `.env.example` (`DSP_GOOGLE_*`, `DSP_MICROSOFT_*`, `DSP_FACEBOOK_*`, `DSP_SMS_PROVIDER`, `DSP_AUTH_PROVIDER_*`, `DSP_AUTH_MFA`, `DSP_WEBAUTHN_*`, Argon2id / lockout knobs). Passkey (WebAuthn) primary sign-in requires the optional `webauthn` package (`pip install "auth[passkey]"`) and, in production, HTTPS with `DSP_WEBAUTHN_RP_ID`/`DSP_WEBAUTHN_ORIGIN` matching the deployed domain. TOTP MFA (`/auth/mfa/{enroll,enable,verify,disable}`, recovery codes) is gated by the same `DSP_AUTH_MFA=true` flag; set `DSP_MFA_SECRET_KEY` (a Fernet key) in production for encrypted-at-rest secret storage — see [ENTERPRISE_AUTH_PLATFORM.md §3d](security/ENTERPRISE_AUTH_PLATFORM.md) for enrollment/recovery/deployment details and `DSP_AUTH_TRUSTED_DEVICE_DAYS` for the "remember this device" TTL.
