"""Immutable domain models for Management Quality Intelligence (FEATURE-002)."""
...
class ManagementConfidence:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.value, (int, float)) or isinstance(self.value, bool):
            raise ManagementQualityValidationError("confidence.value must be numeric")
        ...
        if not basis:
            raise ManagementQualityValidationError("confidence.basis is required")
...
class ManagementScore:
    ...
    def __post_init__(self) -> None:
        ...
        if not status:
            raise ManagementQualityValidationError("score.status is required")
...
class ManagementEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        if not source:
            raise ManagementQualityValidationError("evidence.source is required")
        if not reference:
            raise ManagementQualityValidationError("evidence.reference is required")
...
@dataclass(frozen=True, slots=True)
class ManagementValidationSummary:
    ok: bool
...
@dataclass(frozen=True, slots=True)
class ManagementExplainability:
    evidence: tuple[ManagementEvidence, ...] = ()
    confidence: ManagementConfidence = ManagementConfidence()
...
@dataclass(frozen=True, slots=True)
class ManagementComponentScore:
...
class ManagementAnalysis:
    """Complete Management Quality analysis — explainable, evidence-backed."""

    metadata: ManagementMetadata
    validation: ManagementValidationSummary
    score: ManagementScore
    evidence: tuple[ManagementEvidence, ...]
    confidence: ManagementConfidence
    explainability: ManagementExplainability
    components: tuple[ManagementComponentScore, ...] = ()
    overall_management_rating: ManagementRating | None = None
    summary: str = ""
    strengths: tuple[str, ...] = ()
    weaknesses: tuple[str, ...] = ()
    risks: tuple[str, ...] = ()
    recommendation: str = ""
    weights_used: ManagementWeights | None = None
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "score": self.score.to_dict(),
            "overall_management_score": self.overall_management_score,
            "overall_management_rating": (
                self.overall_management_rating.value
                if self.overall_management_rating is not None
                else None
            ),
            "evidence": [item.to_dict() for item in self.evidence],
            "confidence": self.confidence.to_dict(),
